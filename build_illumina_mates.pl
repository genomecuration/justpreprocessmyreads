...
$sort_version =~ /(\d+)\.\d+$/;
$sort_version = $1;
$sort_exec .= " --parallel=$cpu "
  if $sort_version && $sort_version >= 8 && $cpu > 1;
...
if ($sorted) {
 ...
 system( "cat $infile1 "
  . '|awk \'{OFS="\t"; getline seqs; getline sep; getline quals; print 0$0,seqs,sep,quals}\'| '
  . $sort_exec . ' -S'
  . $buffer
  . ' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2,"+",$4}\'  >'
  . " $infile1.sorted" )
 ...
 system(   "cat $infile1 "
         . '|awk \'{OFS="\t"; getline seqs; print 0$0,seqs}\'| '
         . $sort_exec . ' -S'
         . $buffer
         . ' -k1,1|awk \'{OFS="\n";sub(/^0/,"");print $1,$2}\'  >'
         . " $infile1.sorted" )
...
print
  "Finished with $total mated sequences of a total length of $length b.p.\n";
...
sub process_sorted($$) {
 ...
 while ( my $sid1 = <IN1> ) {
  ...
  if ($is_fasta) {
   ...
   &decide_sort_print( $sid1, $seq1, $suffix1, $qid1, $qlt1,
                       $sid2, $seq2, $suffix2, $qid2, $qlt2 );
